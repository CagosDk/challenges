...
const CHALLENGES_I18N = {
	en: {
		...
		'card.admin_title': 'Challenges – Admin',
		...
		'msg.no_tasks': 'No tasks right now.',
			...
			// Editor UI
			'editor.mode':'Mode', 'editor.mode_admin':'Admin', 'editor.mode_kid':'Kid', 'editor.mode_overview':'Recent tasks',
	...
	da: {
		...
		'shop.title':'Pointshop','shop.open':'Åbn shop','shop.item':'Vare','shop.price':'Pris','shop.icon':'Ikon','shop.image':'Billede','shop.upload':'Upload billede','shop.add_item':'Tilføj vare','shop.history':'Købshistorik','shop.child':'Barn','shop.when':'Tidspunkt','shop.date':'Dato','shop.time':'Tidspunkt','shop.advanced':'Avancerede handlinger','shop.entity':'Enhed','shop.operation':'Handling','shop.add_action':'Tilføj handling','shop.add_delay':'Tilføj delay','shop.steps':'Trin','shop.seconds':'Sekunder','shop.minutes':'Minutter','shop.hours':'Timer','shop.delay':'Forsinkelse','shop.active':'Aktiv','shop.buy':'Køb','shop.bought':'Købt','shop.clear_history':'Ryd historik','confirm.clear_history':'Ryd hele købshistorikken?',
			...
			,
			// Editor UI
			'editor.mode':'Tilstand','editor.mode_admin':'Admin','editor.mode_kid':'Barn','editor.mode_overview':'Seneste opgaver',
			...
			'card.admin_title': 'Challenges – Admin', 'input.new_child_name': 'Nytt barn — namn', 'btn.add_child': 'Lägg till barn',
			...
			// Editor UI
			'editor.mode':'Läge','editor.mode_admin':'Admin','editor.mode_kid':'Barn','editor.mode_overview':'Senaste uppgifter',
			...
			'card.admin_title':'Challenges – Admin','input.new_child_name':'Nytt barn — navn','btn.add_child':'Legg til barn',
			...
			// Editor UI
			'editor.mode':'Modus','editor.mode_admin':'Admin','editor.mode_kid':'Barn','editor.mode_overview':'Siste oppgaver',
			...
			'card.admin_title':'Challenges – Admin','input.new_child_name':'Neues Kind — Name','btn.add_child':'Kind hinzufügen',
			...
			// Editor UI
			'editor.mode':'Modus','editor.mode_admin':'Admin','editor.mode_kid':'Kind','editor.mode_overview':'Aktuelle Aufgaben',
			...
			'card.admin_title':'Challenges – Admin','input.new_child_name':'Nouvel enfant — nom','btn.add_child':'Ajouter un enfant',
...
const CHALLENGES_LANG_ALIAS = { no: 'nb' };
try {
	const __EN = CHALLENGES_I18N.en || {};
	for (const __lang of Object.keys(CHALLENGES_I18N)){
		if (__lang==='en') continue; const __m = CHALLENGES_I18N[__lang] ||= {};
...
function c4kLocalize(key, langOrHass, vars){
	...
	const candidates = [lang, parts[0], CHALLENGES_LANG_ALIAS[lang], CHALLENGES_LANG_ALIAS[parts[0]], 'en'].filter(Boolean);
	let txt = null; for (const c of candidates){ const d=CHALLENGES_I18N[c]; if (d && d[key]) { txt=d[key]; break; } }
	if (!txt) txt = CHALLENGES_I18N.en[key] || key;
...
class ChallengesCard extends LitElement {
		...
		return html`<div class="overlay ${this._iconModalOpen?'open':''}" @click=${e=>{ if (e.target.classList.contains('overlay')) this._iconModalOpen=false; }}>
			<div class="modal" style="max-width: 560px; width: min(95vw, 560px);" @click=${e=>e.stopPropagation()}>
				...
				${(()=>{ 
					...
					const allIcons = [...customIcons, ...CHALLENGES_ICON_SET];
					...
					const base=filter(CHALLENGES_ICON_SET);
...
// Simple GUI editor
class ChallengesCardEditor extends LitElement{
...
window.customCards.push({ type: 'challenges-card', name: 'Challenges (Admin/Profile/Latest)', preview: true, description: 'Combined card – choose Parent, Child, or Latest tasks in the editor' });
